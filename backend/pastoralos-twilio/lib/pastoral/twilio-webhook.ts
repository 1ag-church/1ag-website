...
export async function handleTwilioWebhook(request: Request, config: ServerConfig, store: WebhookStore, kind: 'inbound' | 'status'): Promise<Response> {
  ...
  const generalNumber = string(config, 'PASTORALOS_GENERAL_SMS_NUMBER');
  const acceptedNumber=(value:string)=>value===number||!!generalNumber&&phone(generalNumber)&&generalNumber!==number&&value===generalNumber;
