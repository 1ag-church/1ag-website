...
export function PrayerAutomationTools({state,busy,run,onRefresh}:{state:State;busy:boolean;run:(action:Action)=>Promise<boolean>;onRefresh:()=>Promise<void>}) {
    ...
    {!requests.length?<div className="px-6 pb-6 text-sm muted">No prayer-line requests recorded yet. New requests will appear here as they are received.</div>:<div className="divide-y">
      {requests.slice(0,limit).map(item=>{
        ...
        return <article key={item.id} className="p-6">
          ...
          <details className="mt-4 text-sm" open={displayStatus==='review'||displayStatus==='error'}>
      ...
      {requests.length>limit&&<div className="p-5"><Button variant="outline" size="sm" onClick={()=>setLimit(limit+20)}>Show more requests</Button></div>}
    </div>}
