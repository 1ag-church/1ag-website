...
export function PrayerReviewCard({state,prayerId,busy,run}:{state:State;prayerId:string;busy:boolean;run:(action:Action)=>Promise<boolean>}) {
  ...
  const status=prayer.state==='closed'?'Declined':needsApproval?'Awaiting your decision':uncertain?'Delivery needs attention':sent===deliveries.length&&sent>0?'Sent to prayer chain':state.settings.paused||state.settings.prayerPaused?'Approved · waiting for workflows to resume':'Approved · queued for prayer chain';
  ...
  return <article className="panel p-6 space-y-4">
    ...
    <div><h4 className="font-semibold mb-2">{privateRequest?'Request':'Message to the prayer chain'}</h4>{editing?<Textarea aria-label="Prayer-chain message" value={body} maxLength={6000} rows={6} onChange={event=>setBody(event.target.value)}/>:<p className="callout whitespace-pre-wrap break-words">{body}</p>}</div>
    ...
    {needsApproval&&<>
      ...
      <div className="flex gap-2 flex-wrap">
        {!privateRequest&&<Button disabled={busy||!body.trim()||!recipients.length||!deliveryReady||prayer.sample} onClick={()=>decide(true)}><Check size={16}/>Approve &amp; send</Button>}
        <Button variant="outline" disabled={busy} onClick={()=>decide(false)}><X size={16}/>{privateRequest?'Close private request':'Decline'}</Button>
        {!privateRequest&&<Button variant="ghost" disabled={busy} onClick={()=>setEditing(!editing)}><PenLine size={15}/>{editing?'Done editing':'Edit wording'}</Button>}
      </div>
    </>}
