...
export function applyIncomingSms(previous: State, event: IncomingSms, now = new Date(), line:'prayer'|'general'='prayer'): State {
  ...
  if (optOut === 'START') {
    ...
    s.tasks.unshift({ id: `${id}:consent`, personId: people.length === 1 ? people[0].id : undefined,
      title: 'Review SMS re-enrollment request', detail: 'START/YES received. Confirm permission for church texts before restoring texting. Number suppression remains active.', done: false, private: true });
  }
