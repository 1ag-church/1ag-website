...
export function deliveryConnection(config:BroadcastConfig):DeliveryConnection {
 ...
 return {sms:!!config.twilio?.token,smsFrom:config.twilio?.phone,...(!config.twilio?.token?{smsReason:'The separate communications texting number is not connected yet. Prayer-chain texting continues on its existing number.'}:{}),email,emailFrom:ses?.from??'info@1ag.tv',emailReplyTo:ses?.replyTo??'info@1ag.tv',...(!email?{emailReason:'Amazon SES needs verified sender, production access, credentials, and an unsubscribe contact list.'}:{})};
}
export function readBroadcastConfig(read:(name:string)=>string|undefined,twilio?:BroadcastConfig['twilio']):BroadcastConfig {const general=read('PASTORALOS_GENERAL_SMS_NUMBER')??'',enabled=read('PASTORALOS_GENERAL_SMS_ENABLED')==='true';return {twilio:twilio&&enabled&&/^\+[1-9]\d{7,14}$/.test(general)&&general!==twilio.phone?{...twilio,phone:general}:undefined,ses:{region:read('PASTORALOS_SES_REGION')??'us-east-1',accessKeyId:read('PASTORALOS_SES_ACCESS_KEY_ID')??'',secretAccessKey:read('PASTORALOS_SES_SECRET_ACCESS_KEY')??'',sessionToken:read('PASTORALOS_SES_SESSION_TOKEN'),from:'info@1ag.tv',replyTo:'info@1ag.tv',contactList:read('PASTORALOS_SES_CONTACT_LIST')??'',postalAddress:read('PASTORALOS_EMAIL_POSTAL_ADDRESS')??'',enabled:read('PASTORALOS_SES_ENABLED')==='true'}};}
