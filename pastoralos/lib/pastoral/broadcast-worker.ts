...
export type BroadcastConfig={twilio?:{accountSid:string;token:string;phone:string;enabled?:boolean;general?:boolean};ses?:{region:string;accessKeyId:string;secretAccessKey:string;sessionToken?:string;from:string;replyTo:string;contactList:string;postalAddress:string;enabled:boolean}};
...
export function deliveryConnection(config:BroadcastConfig):DeliveryConnection {
 ...
 return {sms:!!config.twilio?.token&&config.twilio.enabled!==false,smsFrom:config.twilio?.phone,smsGeneral:!!config.twilio?.general,...(config.twilio?.enabled===false?{smsReason:'The general texting line is awaiting Twilio verification. You can save drafts.'}:{}),email,emailFrom:ses?.from??'info@1ag.tv',emailReplyTo:ses?.replyTo??'info@1ag.tv',...(!email?{emailReason:'Amazon SES needs verified sender, production access, credentials, and an unsubscribe contact list.'}:{})};
}
export function readBroadcastConfig(read:(name:string)=>string|undefined,twilio?:BroadcastConfig['twilio']):BroadcastConfig {const generalPhone=read('PASTORALOS_GENERAL_SMS_NUMBER');if(generalPhone&&twilio)twilio={...twilio,phone:generalPhone,general:true,enabled:generalPhone!==twilio.phone&&/^\+[1-9]\d{7,14}$/.test(generalPhone)&&read('PASTORALOS_GENERAL_SMS_ENABLED')==='true'};return {twilio,ses:{region:read('PASTORALOS_SES_REGION')??'us-east-1',accessKeyId:read('PASTORALOS_SES_ACCESS_KEY_ID')??'',secretAccessKey:read('PASTORALOS_SES_SECRET_ACCESS_KEY')??'',sessionToken:read('PASTORALOS_SES_SESSION_TOKEN'),from:'info@1ag.tv',replyTo:'info@1ag.tv',contactList:read('PASTORALOS_SES_CONTACT_LIST')??'',postalAddress:read('PASTORALOS_EMAIL_POSTAL_ADDRESS')??'',enabled:read('PASTORALOS_SES_ENABLED')==='true'}};}
...
export async function sendBroadcastSms(b:Broadcast,t:BroadcastTarget,config:NonNullable<BroadcastConfig['twilio']>,fetcher:typeof fetch=fetch):Promise<string>{
 if(config.enabled===false)throw new ProviderRejected('General texting is awaiting verification.');
