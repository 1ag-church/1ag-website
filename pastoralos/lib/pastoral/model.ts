...
export type Person = { channelPermissions?:ChannelPermissions; id:string; name:string; email:string; phone:string; address:string; household:string; stage:string; assimilation?:boolean; paused:boolean; archived:boolean; prayerMember:boolean; guestSms:boolean; guestEmail:boolean; prayerSms:boolean; firstVisit:string; stageEnteredAt?:string; source:string; note:string; context:number; sample:boolean; completedAt?:string };
...
export type Message = { id:string; program:Program; personId?:string; prayerId?:string; eventId?:string; eventVersion?:number; ruleId?:string; occurrence?:Occurrence; subject:string; body:string; channel:Channel; status:'pending'|'approved'|'held'|'cancelled'; revision:number; createdAt:string; scheduledAt:string; targets:Target[]; approval?:{ revision:number; fingerprint:string; actor:string; at:string; expiresAt:string; sendRequested?:boolean }; reason:string };
export type Delivery = { id:string; messageId:string; revision:number; target:Target; providerId?:string; attemptedAt?:string; status:'sending'|'queued'|'suppressed'|'cancelled'|'simulated'|'sent'|'delivered'|'failed'|'uncertain'; reason:string; at:string };
...
function submitted(s:State,m:Message){return s.deliveries.some(d=>d.messageId===m.id&&['sending','sent','delivered','uncertain'].includes(d.status));}
...
function makeDraft(s:State,p:Person,r:Rule,now:string){
 ...
 if(r.recurrence){
  ...
  if(previous.some(m=>m.occurrence?.anchor===anchor&&((m.status==='pending'||m.status==='held')&&occurrenceWindow(p,r,m)||m.status==='approved'&&m.approval&&m.approval.expiresAt>now&&!s.deliveries.some(d=>d.messageId===m.id&&['sent','delivered','failed','suppressed','cancelled'].includes(d.status)))))return false;
...
export function applyAction(previous:State,a:Action,actor:string,now=new Date()):State{
 ...
 if(a.type==='person.save'){
  ...
  const targets:Target[]=[];for(const member of s.people.filter(x=>x.prayerMember&&channelPermissions(x).sms&&!x.paused&&!x.archived&&x.phone)){if(!targets.some(t=>t.destination===member.phone))targets.push({personId:member.id,destination:member.phone,context:member.context});}
  ...
  const m=find(s.messages,a.id);if(m.program==='guest'&&submitted(s,m))throw Error('This follow-up was already submitted. Check delivery before creating a new message.');if(m.status==='approved')throw Error('This message is already approved.');if(m.status!=='pending'||m.revision!==a.revision)throw Error('This version is not awaiting approval.');
  ...
  m.status='approved';m.approval={revision:m.revision,fingerprint:fingerprint(m),actor,at,expiresAt:expiry,sendRequested:true};
  for(const target of m.targets){const reason=eligible(s,m,target);s.deliveries.push({id:uid(),messageId:m.id,revision:m.revision,target:structuredClone(target),status:reason?'suppressed':'queued',reason:reason??(m.program==='prayer'?'Approved; queued for prayer-chain delivery':'Approved; queued for assimilation delivery'),at});}label=m.program==='prayer'?'Prayer approved and queued for delivery':'Assimilation message approved and queued for delivery';
  ...
  const p=find(s.people,a.personId);const channel=a.channel==='email'?'email':'sms';if(!(channel==='sms'?channelPermissions(p).sms&&p.phone:channelPermissions(p).email&&p.email))throw Error('Guest contact permission required.');s.messages.unshift({id:uid(),program:'guest',personId:p.id,subject:reqText(a.subject,180),body:reqText(a.body,6000),channel,status:'pending',revision:1,createdAt:at,scheduledAt:at,targets:[{personId:p.id,destination:channel==='sms'?p.phone:p.email,context:p.context}],reason:'Personal follow-up'});label='Personal message drafted for approval';
  ...
  let count=0;if(!s.settings.paused&&!s.settings.guestPaused)for(const p of s.people){if(!isAssimilating(p)||p.paused||p.archived||p.completedAt||['Completed','Regular attendee'].includes(p.stage))continue;for(const r of s.rules){if(makeDraft(s,p,r,at))count++;}}label=`Prepared ${count} eligible follow-up drafts; none sent`;
